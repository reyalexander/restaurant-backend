...
class UserViewFilter(filters.BaseFilterBackend):
    def filter_queryset(self, request, queryset, view):
        # parametros especiales del Modelo
        id_role = request.query_params.get("id_role", None)
        if id_role:
            queryset = queryset.filter(id_role=id_role)
...
class RolViewFilter(filters.BaseFilterBackend):
    def filter_queryset(self, request, queryset, view):
        # parametros especiales del Modelo
        # Obtiene los parámetros de consulta (query params) del request
        status = request.query_params.get("status", None)
